...
3. **Verify the solution file:**
   After export, you should see a ZIP file named `<SolutionName>_Unmanaged.zip` in the specified path.
...
2. **Unpack the solution:**
   ...
   ```bash
   pac solution unpack --zipfile "./solutions/<SolutionName>_Unmanaged.zip" --folder "./unpacked-solution"
   ```
...
- **Authenticate to a new environment:**
  ...
  ```bash
  pac auth create --url https://new-org.crm.dynamics.com
  ```
